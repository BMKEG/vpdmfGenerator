...
import edu.isi.bmkeg.vpdmf.builders.ElasticSearchViewBuilder;
...
public class BuildBasicElasticSearchPojos {
	...
	public static void main(String[] args) throws Exception {
		...
		Model firstPomModel = VPDMfGeneratorConverters.readModelFromPom(pomFiles.get(0));
		VpdmfSpec firstSpecs = VPDMfGeneratorConverters.readVpdmfSpecFromPom(firstPomModel);
		VPDMfParser vpdmfP = new VPDMfParser();
		VPDMf top = vpdmfP.buildAllViewsAsClassModel(firstSpecs, model, 
				viewFiles, firstSpecs.getSolrViews());
		...
		ElasticSearchViewBuilder es = new ElasticSearchViewBuilder(top);	
		
		es.setBuildQuestions(false);
		...
		es.buildMavenProject(zip, null, 
				group, artifactId + "-es", version, 
				bmkegParentVersion);

		File buildDir = new File(dAddr + "/pojoModel");
		Converters.unzipIt(zip, buildDir);

		String srcFileName = artifactId + "-es-" + version + "-src.jar";
		String srcDirName = artifactId + "-es";
		...
		logger.info("Spring Data ElasticSearch POJOs source:" + dir.getPath() + "/" + srcDirName);
